...
calcDerivativeCoef(const std::array<RealType, N>& points) noexcept
{
	...
	{
		A(i, 0) = 0; // íóëåâîé ñòîëáåö çàïîëíåí íóëÿìè (êðîìå íóëåâîé ñòðîêè) - ñîîòâåòñòâóåò òî÷êå x0
		for (unsigned int j = 1; j <= N; j++)
			A(i, j) = A(i - 1, j) * points[j - 1] / i; // ýëåìåíòû ìàòðèöû íàïîìèíàþò êîýôôèöèåíòû ðàçëîæåíèÿ ýêñïîíåíòû â ðÿä Òåéëîðà
	}

	Eigen::Vector<RealType, N + 1> b = Eigen::Vector<RealType, N + 1>::Zero();
	b(1) = 1; //  ïî÷òè íóëåâîé ñòîëáåö b ñèñòåìû A*x=b
